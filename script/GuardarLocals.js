...
formulario.addEventListener('submit', (e)=>{
    ...
    function LeerDatos() {
        ...
        cantidad_horas = document.getElementById ('cantidad_horas').value
    

        console.log(vehiculo)
        ValidarData(vehiculo,modelo,tipo,fecha,horas,cantidad_horas)
    ...
    function ValidarData(vehiculo, modelo, tipo, fecha, horas, cantidad_horas) {
        if(vehiculo.length==0 || modelo.length==0 || tipo.length==0 || fecha.length==0 || horas.length==0 ||cantidad_horas.length==0 ||){
